...
type elasticForecastMetadata struct {
	...
	LookAhead   time.Duration `keda:"name=lookAhead,   order=triggerMetadata, default=5m"`
	...
	// ResultsIndexName controls which .ml-anomalies-* index to query. Default is "shared" (.ml-anomalies-shared).
	ResultsIndexName string `keda:"name=resultsIndexName, order=triggerMetadata, default=shared"`
...
func durationToESString(d time.Duration) string {
	...
	for _, c := range []candidate{
		...
		if d%c.factor == 0 {
			return fmt.Sprintf("%d%s", int64(d/c.factor), c.unit)
		}
	}
...
// ensureForecastValid checks whether a new forecast needs to be created (first time and less than forecastRenewThreshold of the window remains).
func (s *elasticForecastScaler) ensureForecastValid(ctx context.Context) error {
	s.mu.Lock()
	id := s.forecastID
	expiry := s.forecastExpiry
	s.mu.Unlock()

	duration := s.metadata.forecastDuration()
	renewThreshold := time.Duration(float64(duration) * forecastRenewThreshold)
	needsRenewal := id == "" || time.Until(expiry) < renewThreshold
	...
	if id == "" {
		...
		s.logger.Info("renewing ML forecast (window near expiry)",
			...
			"remainingWindow", time.Until(expiry).Round(time.Second),
			"renewThreshold", renewThreshold,
		)
	}
	return s.createForecast(ctx)
}

// ensureJobOpen checks the ML job state and opens it if it is closed.
func (s *elasticForecastScaler) ensureJobOpen(ctx context.Context) error {
	statsPath := fmt.Sprintf("/_ml/anomaly_detectors/%s/_stats", s.metadata.JobID)
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, statsPath, nil)
	if err != nil {
		return fmt.Errorf("build job stats request: %w", err)
	}

	resp, err := s.esClient.Perform(req)
	if err != nil {
		return fmt.Errorf("job stats API call: %w", err)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return fmt.Errorf("read job stats response: %w", err)
	}
	if resp.StatusCode >= 300 {
		return fmt.Errorf("job stats API returned HTTP %d: %s", resp.StatusCode, string(body))
	}

	state := gjson.GetBytes(body, "jobs.0.state").String()
	if state == "opened" {
		return nil // already open, nothing to do
	}

	s.logger.Info("ML job is not opened, opening it before forecast",
		"jobID", s.metadata.JobID,
		"currentState", state,
	)

	openPath := fmt.Sprintf("/_ml/anomaly_detectors/%s/_open", s.metadata.JobID)
	openReq, err := http.NewRequestWithContext(ctx, http.MethodPost, openPath, nil)
	if err != nil {
		return fmt.Errorf("build job open request: %w", err)
	}
	openReq.Header.Set("Content-Type", "application/json")

	openResp, err := s.esClient.Perform(openReq)
	if err != nil {
		return fmt.Errorf("job open API call: %w", err)
	}
	defer openResp.Body.Close()

	openBody, err := io.ReadAll(openResp.Body)
	if err != nil {
		return fmt.Errorf("read job open response: %w", err)
	}
	if openResp.StatusCode >= 300 {
		return fmt.Errorf("job open API returned HTTP %d: %s", openResp.StatusCode, string(openBody))
	}
	if !gjson.GetBytes(openBody, "opened").Bool() {
		return fmt.Errorf("job open response did not confirm opened state: %s", string(openBody))
	}

	s.logger.Info("ML job successfully opened", "jobID", s.metadata.JobID)
	return nil
}
...
func (s *elasticForecastScaler) createForecast(ctx context.Context) error {
	// First ensure that the job is open
	if err := s.ensureJobOpen(ctx); err != nil {
		return fmt.Errorf("ensure job is open before forecast: %w", err)
	}

	duration := s.metadata.forecastDuration()
	esDuration := durationToESString(duration)
	...
	s.mu.Lock()
	// Promote the current forecast to "previous" before overwriting it.
	if s.forecastID != "" {
...
func (s *elasticForecastScaler) getForecastedValue(ctx context.Context) (float64, error) {
	...
	// The new forecast is not yet indexed, use previous forecast (it still covers now+lookAhead because its window is 2×lookAhead)
	if previousID != "" {
	...
	// Return 0 when no forecast is present, so we don't get warnings.
...
func (s *elasticForecastScaler) queryForecastBucket(ctx context.Context, forecastID string) (float64, bool, error) {
	...
	query := map[string]interface{}{
		...
		"query": map[string]interface{}{
			"bool": map[string]interface{}{
				"filter": []interface{}{
					...
					// Highest bucket timestamp ≤ target covers the target moment.
		...
		// Descending: first result is the bucket closest to (and ≤) targetMs.
		"sort": []interface{}{
	...
	index := fmt.Sprintf(".ml-anomalies-%s", s.metadata.ResultsIndexName)
...
func (s *elasticForecastScaler) GetMetricsAndActivity(
	ctx context.Context,
	metricName string,
	...
	if err := s.ensureForecastValid(ctx); err != nil {
		return []external_metrics.ExternalMetricValue{}, false,
			fmt.Errorf("error ensuring ML forecast is valid: %w", err)
	}

	val, err := s.getForecastedValue(ctx)
	if err != nil {
		return []external_metrics.ExternalMetricValue{}, false,
			fmt.Errorf("error reading elastic forecast value: %w", err)
	}

	metric := GenerateMetricInMili(metricName, val)
	return []external_metrics.ExternalMetricValue{metric},
		val > s.metadata.ActivationTargetValue,
		nil
}
