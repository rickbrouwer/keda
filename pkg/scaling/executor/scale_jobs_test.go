...
func TestAccurateScalingStrategy(t *testing.T) {
	...
	// maxScale doesn't exceed MaxReplicaCount
	assert.Equal(t, int64(1), maxScaleValue(strategy.GetEffectiveMaxScale(3, 2, 0, 5, 1))) // 3 - (2+0) = 1
	assert.Equal(t, int64(3), maxScaleValue(strategy.GetEffectiveMaxScale(5, 2, 0, 5, 1))) // min(5,5) - (2+0) = 3

	// Test with pending jobs, should account for both running and pending
	assert.Equal(t, int64(0), maxScaleValue(strategy.GetEffectiveMaxScale(3, 2, 2, 10, 1))) // 3 - (2+2) = -1 → 0
	assert.Equal(t, int64(0), maxScaleValue(strategy.GetEffectiveMaxScale(5, 4, 2, 5, 1)))  // min(5,5) - (4+2) = -1 → 0

	// Test cases where we can still scale
	assert.Equal(t, int64(2), maxScaleValue(strategy.GetEffectiveMaxScale(5, 2, 1, 10, 1))) // 5 - (2+1) = 2
}
