...
func setupElasticForecast(t *testing.T) {
	...
	execAndVerify(t, "create ML job",
		...
		func(body string) error {
			// Response contains the job_id on success.
	...
	// Step 9: wait until the datafeed has finished processing the historical.
...
// execAndVerify runs a shell command, captures stdout, and calls validate. The test is failed immediately if the command errors or validate returns a non-nil error.
...
func activateTrialAndVerify(t *testing.T) {
	...
	t.Logf("[activate trial] response: %s", truncate(fmt.Sprintf("%s", out), 300))
...
// waitForForecastDocs polls .ml-anomalies-shared until at least one model_forecast document exists for the test job.
func waitForForecastDocs(t *testing.T) {
	t.Helper()

	// Allow up to 3 minutes: first-time forecast on a 120-bucket model can
	// take longer than on a tiny model.
...
// debugForecastIndex logs the raw contents of the forecast index so we can verify that (a) forecast documents were created, (b) what timestamps they
// carry, and (c) whether the query the scaler uses would match them.
func debugForecastIndex(t *testing.T) {
	t.Helper()
	t.Log("--- [debug] inspecting forecast index ---")

	// 1. Count all model_forecast documents in the shared index.
	countQuery := `{"query":{"term":{"result_type":"model_forecast"}}}`
	out, err := ExecuteCommand(fmt.Sprintf(
		"%s -XGET 'http://127.0.0.1:9200/.ml-anomalies-shared/_count' -d '%s'",
		forecastKubectlExecCmd, countQuery,
	))
	if err == nil {
		t.Logf("[debug] total model_forecast doc count: %s", truncate(fmt.Sprintf("%s", out), 300))
	}

	// 2. Fetch the first 3 forecast buckets sorted by timestamp ascending so
	//    we can see what timestamps the forecast actually covers.
	firstBucketsQuery := `{
		"size": 3,
		"query": {"term": {"result_type": "model_forecast"}},
		"sort": [{"timestamp": {"order": "asc"}}],
		"_source": ["forecast_id","job_id","timestamp","forecast_prediction","result_type"]
	}`
	out, err = ExecuteCommand(fmt.Sprintf(
		"%s -XGET 'http://127.0.0.1:9200/.ml-anomalies-shared/_search' -d '%s'",
		forecastKubectlExecCmd, firstBucketsQuery,
	))
	if err == nil {
		t.Logf("[debug] first 3 forecast buckets (asc): %s", truncate(fmt.Sprintf("%s", out), 600))
	}

	// 3. Show now, now+5m in millis so we can compare to bucket timestamps.
	now := time.Now()
	targetMs := now.Add(5 * time.Minute).UnixMilli()
	t.Logf("[debug] now = %s (%d ms)", now.Format(time.RFC3339), now.UnixMilli())
	t.Logf("[debug] now+5m (targetMs) = %s (%d ms)", now.Add(5*time.Minute).Format(time.RFC3339), targetMs)

	// 4. Run the exact scaler query (lte targetMs, with forecast_id filter).
	scalerQuery := fmt.Sprintf(`{
		"size": 3,
		"query": {"bool": {"filter": [
			{"term": {"job_id": "%s"}},
			{"term": {"result_type": "model_forecast"}},
			{"range": {"timestamp": {"lte": %d}}}
		]}},
		"sort": [{"timestamp": {"order": "desc"}}],
		"_source": ["forecast_id","timestamp","forecast_prediction"]
	}`, forecastJobID, targetMs)
	out, err = ExecuteCommand(fmt.Sprintf(
		"%s -XGET 'http://127.0.0.1:9200/.ml-anomalies-shared/_search' -d '%s'",
		forecastKubectlExecCmd, scalerQuery,
	))
	if err == nil {
		t.Logf("[debug] scaler query (lte now+5m): %s", truncate(fmt.Sprintf("%s", out), 600))
	}

	// 5. Query without lte filter, shows all buckets regardless of timestamp.
	allBucketsQuery := fmt.Sprintf(`{
		"size": 3,
		"query": {"bool": {"filter": [
			{"term": {"job_id": "%s"}},
			{"term": {"result_type": "model_forecast"}}
	...
	out, err = ExecuteCommand(fmt.Sprintf(
		"%s -XGET 'http://127.0.0.1:9200/.ml-anomalies-shared/_search' -d '%s'",
		forecastKubectlExecCmd, allBucketsQuery,
	))
	if err == nil {
		t.Logf("[debug] ALL forecast buckets (no lte, asc): %s", truncate(fmt.Sprintf("%s", out), 800))
	}
}
